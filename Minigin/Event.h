...
{
	consteval static unsigned int _calculate(const char* text, unsigned int& val)
	{
		const unsigned int character = sdbm_hash<length - 1>::_calculate(text, val);
	...
	{
		unsigned int val = 0;
		const auto character = _calculate(text, val);
		return character + (val << 6) + (val << 16) - val;
	}
};
