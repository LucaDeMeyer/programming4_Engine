...
void Tron::LevelManager::LoadMenu(dae::Scene& scene)
{
    ...
    auto playBtn = std::make_unique<dae::GameObject>();
    playBtn->GetTransform()->SetLocalPosition({ 1024/2, 576/2, 0 });
    playBtn->AddComponent<dae::TextComponent>()->SetText("Play");
    playBtn->GetComponent<dae::TextComponent>()->SetFont("TRON.TTF", 25);
	playBtn->GetComponent<dae::TextComponent>()->SetColor(255, 255, 255, 255);
    playBtn->AddComponent<dae::BoxColliderComponent>(glm::vec4{ 0, 0, 200, 50 });
    auto btnComp = playBtn->AddComponent<dae::ButtonComponent>();
    btnComp->SetCallback([this]() {
        this->LoadLevel("TestLevel.csv", LevelCategory::Game);
        });
	scene.Add(std::move(playBtn));
}
